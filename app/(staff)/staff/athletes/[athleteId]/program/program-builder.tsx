...
const LOAD_MODES: LoadMode[] = ["lb", "kg", "pct", "bw"];
...
export function ProgramBuilder({
  ...
  function changeLoadMode(sectionIdx: number, exUid: string, next: LoadMode) {
    patchExercise(sectionIdx, exUid, (e) => {
      const prev = e.loadMode;
      const sets = e.sets.map((s) => {
        if (s.load === "") return s;
        const n = Number(s.load);
        if (Number.isNaN(n)) return s;
        if (prev === "lb" && next === "kg") return { ...s, load: String(lbToKg(n)) };
        if (prev === "kg" && next === "lb") return { ...s, load: String(kgToLb(n)) };
        return s;
      });
      return { ...e, loadMode: next, sets };
    });
  }
  ...
  function addSet(sectionIdx: number, exUid: string) {
    patchExercise(sectionIdx, exUid, (e) => ({
      ...e,
      sets: [...e.sets, e.sets[e.sets.length - 1] ?? { target: "8", load: "" }],
    }));
  }
          ...
          <section key={section.title} className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
              {section.exercises.map((ex, exIdx) => {
                ...
                const prevLinked = exIdx > 0 && section.exercises[exIdx - 1].linkNext;
                ...
                return (
                  <div key={ex.uid} className="relative">
                    {/* superset rail */}
                    {prevLinked ? (
                      <span className="absolute -top-2 bottom-auto left-[1.35rem] h-2 w-0.5 bg-brand/60" />
                    ) : null}
                    <Card
                      className={cn(
                    ...
                    >
                      <CardContent className="flex flex-col gap-3 p-4">
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={cn(
                              "flex h-7 w-9 items-center justify-center rounded-md font-mono text-xs font-bold",
                              inSuperset
                        ...
                        <Input
                          ...
                          placeholder="Exercise instructions (shown to the athlete)…"
                          className="h-8 border-dashed bg-transparent text-xs"
                          onChange={(ev) =>
                          ...
                          <table className="w-full min-w-[420px] text-sm">
                            <thead>
                              <tr className="text-left text-xs text-muted-foreground">
                                <th className="w-14 pb-1.5 font-medium">Set</th>
                                <th className="w-32 pb-1.5 font-medium">
                                  {ex.repMode === "reps"
                                    ? "Reps"
                                    : ex.repMode === "time"
                                      ? "Time"
                                ...
                                <th className="pb-1.5 font-medium">
                                  <select
                                    value={ex.loadMode}
                                    className="rounded-md border border-border bg-surface px-2 py-1 text-xs font-medium focus-visible:outline-none"
                                    onChange={(ev) =>
                                      changeLoadMode(
                                        sectionIdx,
                                        ex.uid,
                                        ev.target.value as LoadMode,
                                      )
                                    }
                            ...
                            <tbody>
                              {ex.sets.map((set, setIdx) => (
                                <tr key={setIdx} className="border-t border-border/60">
                                  <td className="tnum py-1.5 pr-2 text-xs font-bold text-muted-foreground">
                                    {setIdx + 1}
                                  </td>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {ex.loadMode === "pct" && def?.referenceMax ? (
                            <span className="text-xs text-muted-foreground">
                              % of{" "}
...
function ExercisePicker({
  ...
  const allTags = useMemo(() => {
    ...
    library.forEach((e) => e.tags.forEach((t) => counts.set(t, (counts.get(t) ?? 0) + 1)));
