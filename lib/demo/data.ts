...
export interface Invoice {
  ...
  status:
    ...
    | "due"
    | "overdue"
    | "upcoming" // scheduled — queued to send on `scheduledFor` (R16)
    | "draft" // round 16: not yet sent, still editable
    | "canceled"
    | "partial"
    | "refunded";
  ...
  /* ---- Round 16: invoice-management workflow ---- */
  /** When the invoice was emailed; absent on drafts/scheduled. Non-draft
   *  seeds without it are treated as sent on `issuedAt`. */
  sentAt?: string;
  /** Scheduled invoices ("upcoming"): the future send date. */
  scheduledFor?: string;
  /** Archived rows leave every filter except Archived. */
  archived?: boolean;
  /** The invoice email bounced — resend after fixing the address. */
  undelivered?: boolean;
  undeliveredNote?: string;
  /** Generated by a recurring series (rs-*). */
  recurringSeriesId?: string;
  /** Last payment-reminder date (Send Reminder action). */
  remindedAt?: string;
}
 ...
 *  Round 16 (Q6): offline methods staff record manually include e-transfer. */
...
export const invoices: Invoice[] = [
  ...
  { id: "inv-3", athleteId: "ath-ty", athleteName: "Tyler Brooks", plan: "Academy", amountCents: 26000, issuedAt: at(-11), dueDate: at(3), status: "upcoming", scheduledFor: at(1), method: "Square" },
  ...
  { id: "inv-6", athleteId: "ath-maya", athleteName: "Maya Okafor", plan: "Academy", amountCents: 26000, issuedAt: at(-8), dueDate: at(6), status: "upcoming", scheduledFor: at(2), method: "Square" },
  ...
  // Round 16 (Q2): one seed per new workflow state so every filter demos.
  { id: "inv-9", athleteId: "ath-priya", athleteName: "Priya Nair", plan: "Pro Track", amountCents: 34000, issuedAt: at(-1), dueDate: at(13), status: "draft", method: "Square" },
  { id: "inv-10", athleteId: "ath-leo", athleteName: "Leo Martin", plan: "Executive", amountCents: 30000, issuedAt: at(-6), dueDate: at(2), status: "due", sentAt: at(-6), undelivered: true, undeliveredNote: "Email bounced — leo.martin@example.com rejected the message.", method: "Square" },
  { id: "inv-11", athleteId: "ath-marcus", athleteName: "Marcus Hale", plan: "Academy", amountCents: 26000, issuedAt: at(-64), dueDate: at(-50), status: "canceled", canceledAt: at(-48), archived: true, method: "Square" },
...
export const recurringSeriesSeed: RecurringSeries[] = [
  {
    id: "rs-ren",
    athleteId: "ath-ren",
...
/** Round 16 (Q5): public share link for an invoice (no login needed). */
